...
void QuadEstimatorEKF::UpdateFromIMU(V3F accel, V3F gyro)
{
  ...
  Quaternion<float> Q_current = Quaternion<float>::FromEuler123_RPY(rollEst, pitchEst, ekfState(6));
  // In order to get the new Euler angles, we want to generate a quaternion derivative from the angular velocities like so:
  //  Qdot = 0.5 * Q * [0; omega];
  // We can then integrate this into the new quaternion:
  //  Qnew = Qcurrent + Qdot * dt;
  // This is all encapsulated into the 'IntegrateBodyRate' method:
  Quaternion<float> Q_new = Q_current.IntegrateBodyRate(gyro, dtIMU);
...
VectorXf QuadEstimatorEKF::PredictState(VectorXf curState, float dt, V3F accel, V3F gyro)
{
  ...
  const V3F imu_i = attitude.Rotate_BtoI(accel) + V3F(0, 0, -9.81);

  predictedState(0) += curState(3) * dt;
  predictedState(1) += curState(4) * dt;
  predictedState(2) += curState(5) * dt;
  predictedState(3) += imu_i.x * dt;
  predictedState(4) += imu_i.y * dt;
  predictedState(5) += imu_i.z * dt;
...
void QuadEstimatorEKF::Predict(float dt, V3F accel, V3F gyro)
{
  ...
  // Rotate the acceleration into the world frame and account for gravity:
  const Quaternion<float> w_Q_b = Quaternion<float>::FromEuler123_RPY(rollEst, pitchEst, ekfState(6));
  const V3F accel_b = accel + w_Q_b.Rotate_ItoB(V3F(0, 0, -9.81));

  const Eigen::Vector3f jac_sub = RbgPrime * Eigen::Vector3f(accel_b.x, accel_b.y, accel_b.z);

  // Update the jacobian based on the acceleration.
  gPrime(VELX, YAW) = jac_sub.x() * dt;
  gPrime(VELY, YAW) = jac_sub.y() * dt;
  gPrime(VELZ, YAW) = jac_sub.z() * dt;
...
void QuadEstimatorEKF::UpdateFromGPS(V3F pos, V3F vel)
{
  ...
  for (size_t i = 0; i < zFromX.size(); ++i)  {
    zFromX(i) = ekfState(i);
  }
